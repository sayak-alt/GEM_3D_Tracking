...
void GEMs_residuals_y() {
    std::vector<float> z1, z2, z3;
    std::vector<float> x1, x2, x3;

    // Histograms
    TH1D *x1_uncorrected_angle_diff = new TH1D("y1_uncorrected_angle_diff", "Y_{1} Difference", 200, -10, 10);
    TH1D *x2_uncorrected_angle_diff = new TH1D("y2_uncorrected_angle_diff", "Y_{2} Difference", 200, -10, 10);
    TH1D *x3_uncorrected_angle_diff = new TH1D("y3_uncorrected_angle_diff", "Y_{3} Difference", 200, -10, 10);
    
    TH1D *x1_corrected_angle_diff = new TH1D("y1_corrected_angle_diff", "Y_{1} Difference", 200, -5, 5);
    TH1D *x2_corrected_angle_diff = new TH1D("y2_corrected_angle_diff", "Y_{2} Difference", 200, -5, 5);
    TH1D *x3_corrected_angle_diff = new TH1D("y3_corrected_angle_diff", "Y_{3} Difference", 200, -5, 5);
    ...
    TF1 *f11 = new TF1("f1", "gaus", -5, 5);
    f1->SetParameters(1, 1);
    TF1 *f21 = new TF1("f2", "gaus", -5, 5);
    f2->SetParameters(1, 1);
    TF1 *f31 = new TF1("f3", "gaus", -5, 5);
    f3->SetParameters(1, 1);

    // Read data from file
    std::ifstream infile("sorted_data_file_run_number_y1_y2_y3.txt");
    if (!infile.is_open()) {
    ...
    c1->Divide(3,1);
    x3_uncorrected_angle_diff->GetXaxis()->SetTitle("Y_{3}^{measured} - Y_{3}^{calculated} (mm)");
    x3_uncorrected_angle_diff->GetYaxis()->SetTitle("Counts");
    x1_uncorrected_angle_diff->GetXaxis()->SetTitle("Y_{1}^{measured} - Y_{1}^{calculated} (mm)");
    x1_uncorrected_angle_diff->GetYaxis()->SetTitle("Counts");
    x2_uncorrected_angle_diff->GetXaxis()->SetTitle("Y_{2}^{measured} - Y_{2}^{calculated} (mm)");
    ...
    c2->Divide(3,1);
    x3_corrected_angle_diff->GetXaxis()->SetTitle("Y_{3}^{measured} - Y_{3}^{calculated} (mm)");
    x3_corrected_angle_diff->GetYaxis()->SetTitle("Counts");
    x1_corrected_angle_diff->GetXaxis()->SetTitle("Y_{1}^{measured} - Y_{1}^{calculated} (mm)");
    x1_corrected_angle_diff->GetYaxis()->SetTitle("Counts");
    x2_corrected_angle_diff->GetXaxis()->SetTitle("Y_{2}^{measured} - Y_{2}^{calculated} (mm)");
