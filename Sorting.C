...
//int main()
{
//Create a output txt file and name it
    Double_t hqdc10;
    ofstream outfile;
       outfile.open("output_file_run_3912_x2y2_GEM-II.txt");//GEM-I
   //     outfile.open("output_file_run_3750_x2y2_GEM-II.txt");//GEM-I
    //      outfile.open("output_file_run_3750_x3y3_GEM-III.txt");//GEM-I
...
*/
   ...
   TString infile = "SelectPulse_3910_x2y2.txt"; //for GEM-I
//    TString infile = "SelectPulse_3417_x2y2.txt"; //for GEM-I
...
//    map<int, TH1D*> xhits;//xhits in GEM plane //x3only
	
    TH1D *hx = new TH1D ("hx","hx",1000,-1000,1000);
    TH1D *hy = new TH1D ("hy","hy",1000,-1000,1000);
...
//    		T->SetBranchAddress(Form("sbs.sbuscint.hadc%d",ij_qdc), &hqdc[ij_qdc]);
	...
	for(int detnumber=0;detnumber<NDet;detnumber++)
	{
		...
		//const int yapv = ystrip[detnumber]/128;
    		cout<<"Det "<<detnumber+3<<"  xapv  "<<xstrip[detnumber]/128<<" yapv "<<ystrip[detnumber]/128<<endl;
...
//        xhits[detnumber]=new TH1D("Det 3 X hits","Det 3 X hits",xstrip[detnumber],-0.5,xstrip[detnumber]-0.5); //x3 only
        ...
        /*
    		T->SetBranchAddress(Form("sbs.gems.x%d.nch",detnumber+3), &x_strip[detnumber]);
		T->SetBranchAddress(Form("sbs.gems.y%d.nch",detnumber+3), &y_strip[detnumber]);
    		T->SetBranchAddress(Form("sbs.gems.x%d.strip",detnumber+3), xstripID[detnumber]);
    		T->SetBranchAddress(Form("sbs.gems.y%d.strip",detnumber+3), ystripID[detnumber]);
         */
        //change the x3 and y3 to x2, y2 and x1, y1 depending on what GEM data you are using
        T->SetBranchAddress(Form("sbs.gems.x2.nch",detnumber+1), &x_strip[detnumber]);
        T->SetBranchAddress(Form("sbs.gems.y2.nch",detnumber+1), &y_strip[detnumber]);
        T->SetBranchAddress(Form("sbs.gems.x2.strip",detnumber+1), xstripID[detnumber]);
        T->SetBranchAddress(Form("sbs.gems.y2.strip",detnumber+1), ystripID[detnumber]);
        T->SetBranchAddress("sbs.sbuscint.ladc0", &hqdc10);

        
    		for(int ij=0;ij<6;ij++)
		{
            /*
      			T->SetBranchAddress(Form("sbs.gems.x%d.adc%d",detnumber+3,ij),xadc[detnumber][ij]);
      			T->SetBranchAddress(Form("sbs.gems.y%d.adc%d",detnumber+3,ij),yadc[detnumber][ij]);
             */
                  T->SetBranchAddress(Form("sbs.gems.x2.adc%d",ij),xadc[detnumber][ij]);
                  T->SetBranchAddress(Form("sbs.gems.y2.adc%d",ij),yadc[detnumber][ij]);
  		}
	}

        if(isOut)
	{
		TFile* rootoutfile =  new TFile(Form("ana_run_%d.root",argc),"RECREATE");
		//rootoutfile->cd();
...
//    					if(xintegral[128*pk+ik]>(xcentroid[detnumber][128*pk+ik]+9*xsigma[detnumber][128*pk+ik]))
                        
                        if(xintegral[128*pk+ik]>(xcentroid[detnumber][128*pk+ik]+(5)*xsigma[detnumber][128*pk+ik])) //testing new sigma values
    					//if(corrch>0)
    					{
						if(ij==nevent-1){
						xhitHisto[detnumber][nxhits] = new TH1D(Form("hx_%d_%d_%d",detnumber+3,ij,(Int_t)xstripID[detnumber][128*pk+ik]),Form("Det %d XEvent %d strip %d",detnumber+3,ij,(Int_t)xstripID[detnumber][128*pk+ik]),6,0,6); 
						...
						C_xhits[detnumber][nxhits] = new TCanvas(Form("Cx_%d_%d_%d",detnumber+3,ij,(Int_t)xstripID[detnumber][128*pk+ik]),Form("Det %d Canvas XEvent %d strip %d",detnumber+3, ij,(Int_t)xstripID[detnumber][128*pk+ik]),500,500); 
...
//    					if(yintegral[128*pk+ik]>(ycentroid[detnumber][128*pk+ik]+10*ysigma[detnumber][128*pk+ik]))
                        if(yintegral[128*pk+ik]>(ycentroid[detnumber][128*pk+ik]+(5)*ysigma[detnumber][128*pk+ik])) // r new ysigma values
    					//if(corrch>0)
	    				{
						if(ij==nevent-1){
						yhitHisto[detnumber][nyhits] = new TH1D(Form("hy_%d_%d_%d",detnumber+3,ij,(Int_t)ystripID[detnumber][128*pk+ik]),Form("Det %d YEvent %d strip %d",detnumber+3,ij,(Int_t)ystripID[detnumber][128*pk+ik]),6,0,6); 
						for(int mn = 0;mn<6;mn++)
						{
						...
						C_yhits[detnumber][nyhits] = new TCanvas(Form("Cy_%d_%d_%d",detnumber+3,ij,(Int_t)ystripID[detnumber][128*pk+ik]),Form("Det %d Canvas YEvent %d strip %d",detnumber+3,ij,(Int_t)ystripID[detnumber][128*pk+ik]),500,500);
...
//filling the output txt files with data

//			if(xcentroid1>0 && ycentroid1>0)
            if(xcentroid1> 0 && ycentroid1> 0) //for x3 only (or only x)
			{
             ...
             //   hits[detnumber]->Fill(xcentroid1,1); //for x3 only (or only x)
               outfile << "  "<<ij<<"   "<<xcentroid1<<"   "<<ycentroid1<<"  "<<xcharge<<"  "<<ycharge<< " " << endl;
 //               outfile << "  "<<ij<<"   "<<xcentroid1<<"   "<<xcharge<<" " << hqdc10 << endl; //for x3 only (or only x)
				h_charge[detnumber]->Fill(xcharge,ycharge);
//                xhits[detnumber]->Fill(xcentroid1); //x3only (this was here before I didn't use this)
			}
            /*
*/
		...
		//Filling the output root file, dont use this
...
//		if(qdccharge>155 && xposition[0]>0 && xposition[1]>0 && xposition[2]>0 && yposition[0]> 0 && yposition[1]>0 && yposition[2]>0)
    ...
    //Lables for the output root file

    outfile << "Entries/Events  " <<"  xcentroid "<<"  ycentroid  "<<" xcharge "<<" ycharge "<< endl;
//    outfile << "Entries/Events  " <<"  xcentroid "<<" xcharge "<< endl; //x3 only
    /*
*/
	...
	for(int detnumber=0;detnumber<NDet;detnumber++)
	{
		...
		C_multiplicity[detnumber] = new TCanvas(Form("Det_%d_multiplicity",detnumber+3),Form("Det %d multiplicity",detnumber+1),800,800);
		...
		h_totcharge[detnumber]->GetXaxis()->SetTitle("Total Charge");
        ...
        TCanvas *can_new = new TCanvas();
        hx->Draw("");
        TCanvas *can_new_1 = new TCanvas();
        hy->Draw("");
        
//        xhits[detnumber]->Draw(""); //x3only
	}
	//return 0;
	if(isOut)
	{
